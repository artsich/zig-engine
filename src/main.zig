...
fn updateEditor() void {
    ...
    if (state.touch_id > 0) {
        ...
        if (rl.isMouseButtonPressed(rl.MouseButton.mouse_button_left)) {
            editor_gizmo.tryActivate(state.main_camera);
            editor_gizmo.updateInitialValues();
        }
        
        
        editor_gizmo.activate(mode.translation);
        editor_gizmo.activate(mode.rotation);
        
        if (rl.isKeyPressed(rl.KeyboardKey.key_r)) {
            editor_gizmo.updateInitialValues();
        }
        
        gizmo_active = editor_gizmo.selected_axis.selected();

        if(gizmo_active) {
            if (rl.isMouseButtonDown(rl.MouseButton.mouse_button_left)) {
                editor_gizmo.translate(state.main_camera);
            }
            ...
            if (rl.isMouseButtonReleased(rl.MouseButton.mouse_button_left) or
                rl.isKeyReleased(rl.KeyboardKey.key_r)
            ) {
                if (editor_gizmo.hasChanged()) {
                    addAction(.{
    ...
    if (!gizmo_active and rl.isMouseButtonPressed(rl.MouseButton.mouse_button_left)) {
